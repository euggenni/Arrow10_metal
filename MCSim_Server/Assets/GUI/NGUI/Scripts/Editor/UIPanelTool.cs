...
public class UIPanelTool : EditorWindow
{
	...
	static List<NGUIPanel> GetListOfPanels ()
	{
		NGUIPanel[] panels = Resources.FindObjectsOfTypeAll(typeof(NGUIPanel)) as NGUIPanel[];
		if (panels == null || panels.Length == 0) return null;

		List<NGUIPanel> list = new List<NGUIPanel>();
	...
	static void GetWidgets (Transform t, List<UIWidget> widgets)
	{
		for (int i = 0; i < t.childCount; ++i)
		{
			...
			else if (child.GetComponent<NGUIPanel>() == null) GetWidgets(child, widgets);
		}
	}
	...
	static List<UIWidget> GetWidgets (NGUIPanel panel)
	{
	...
	static void SetActiveState (Transform t, bool state)
	{
		...
		{
			Transform child = t.GetChild(i);
			//if (child.GetComponent<NGUIPanel>() != null) continue;
