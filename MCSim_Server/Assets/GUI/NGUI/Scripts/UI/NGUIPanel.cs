...
public class NGUIPanel : MonoBehaviour
{
	...
	/// <summary>
	/// Helper function that recursively sets all childrens' game objects layers to the specified value, stopping when it hits another NGUIPanel.
	/// </summary>
	...
	static void SetChildLayer (Transform t, int layer)
	{
		for (int i = 0; i < t.childCount; ++i)
		{
			...
			if (child.GetComponent<NGUIPanel>() == null)
			{
	...
	static public NGUIPanel Find (Transform trans, bool createIfMissing)
	{
		NGUIPanel panel = null;

		while (panel == null && trans != null)
		{
			panel = trans.GetComponent<NGUIPanel>();
			if (panel != null) break;
		...
		{
			panel = trans.gameObject.AddComponent<NGUIPanel>();
	...
	/// <summary>
	/// Find the NGUIPanel responsible for handling the specified transform, creating a new one if necessary.
	/// </summary>

	static public NGUIPanel Find (Transform trans) { return Find(trans, true); }
}