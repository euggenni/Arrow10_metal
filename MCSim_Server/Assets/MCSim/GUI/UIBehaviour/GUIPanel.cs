...
public class GUIPanel : GUIObject
{
	...
	public bool SavePageInHistory = true;
	...
	public virtual void Awake()
	{
		...
		var panel = GetComponent<NGUIPanel>();
	...
	{
		if (!SubMenu)
			transform.localPosition += MoveOnShow * 300f;
	...
	private void MoveAndDestroy(Vector3 direction, float t)
	{
		...
		{
			tc = uiWidget.gameObject.GetComponent<TweenColor>();
			if (!tc) tc = uiWidget.gameObject.AddComponent<TweenColor>();

			tc.from = uiWidget.color;
