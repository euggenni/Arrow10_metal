...
public enum GIZMO_MODE { TRANSLATE = 0, ROTATE = 1, SCALE = 2 }
public enum GIZMO_AXIS { NONE, X, XY, XZ, Y, YZ, Z }

public class GizmoController : MonoBehaviour
{
    public static GizmoController Controller
    {
        get { return GameObject.Find("GizmoAdvanced").GetComponent<GizmoController>(); }
    }
    ...
    //Snap settings
    public float SnappedRotationSpeed = 500;
    public float SnappedScaleMultiplier = 4;
    ...
    private GUISpinner AxisSpinner = new GUISpinner();
    private GUISpinner SnapSpinner = new GUISpinner();
    ...
    void Start()
    {
        ...
        if (Camera.isOrthoGraphic)
            _ortoSize = Camera.orthographicSize;
    ...
    //Set's snapping mode
    // Pass in bool  True=Snap on, False=Snap off
    ...
    void ResetTransformToSelectedObject()
    {
        if (_selectedObject == null)
            return;
    ...
    void SetAllowedModes(bool move, bool rotate, bool scale)
    {
        AllowTranslate = move;
        AllowRotate = rotate;
        AllowScale = scale;
    ...
    void ResetTransformations(bool resetPosition)
    {
        if (resetPosition)
        {
        ...
        if (_showGizmo && ShowGizmoControlWindow)
        {
            if (AllowTranslate || AllowRotate || AllowScale)
            {
                ...
                if (ControlRect.Contains(Event.current.mousePosition))
                    ...
                    GUI.color = new Color(GUI.color.r, GUI.color.g, GUI.color.b, .8f);
    ...
    void DrawGizmoControls(Rect groupRect)
    {
        ...
        switch (_mode)
        {
            case GIZMO_MODE.TRANSLATE:
                if (AllowTranslate)
                {
                    transform.position = new Vector3(AxisSpinner.Draw(new Rect(0, 25, innerRect.width, 20), "X:", transform.position.x),
            ...
            case GIZMO_MODE.ROTATE:
                if (AllowRotate)
                {
                    ...
                    {
                        transform.eulerAngles = new Vector3(AxisSpinner.Draw(new Rect(0, 25, innerRect.width, 20), "X:", transform.rotation.eulerAngles.x),
                        AxisSpinner.Draw(new Rect(0, 47, innerRect.width, 20), "Y:", transform.rotation.eulerAngles.y),
                        AxisSpinner.Draw(new Rect(0, 69, innerRect.width, 20), "Z:", transform.rotation.eulerAngles.z));
            ...
            case GIZMO_MODE.SCALE:
                if (AllowScale)
                {
                    _selectedObject.localScale = new Vector3(AxisSpinner.Draw(new Rect(0, 25, innerRect.width, 20), "X:", _selectedObject.localScale.x),
    ...
    void SetDragHilight(bool SetDrag)
    {
        if (_selectedAxis == null)
            return;
    ...
    void Update()
    {
        ...
        if(Camera.isOrthoGraphic)
        {
            //Debug.Log("SZ:" + _ortoSize);
            float dif = _ortoSize/Camera.orthographicSize;
            transform.localScale = _startSize/dif; // Коэффициент от разрешения экрана
        }
        else
        {
            float distance = Mathf.Abs(Vector3.Distance(Camera.transform.position, transform.position));
            transform.localScale = new Vector3(-1, 1, 1) * (distance / 8);
        }
        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity, layerMask) && !_ignoreRaycast)
        {
            ...
            if (Input.GetMouseButtonUp(0))
            {
                ...
                //Camera.transform.SendMessage("setEnabled", true);
            }//if		
        }//if
        else
        {
        ...
        if (_activeAxis != GIZMO_AXIS.NONE && Input.GetMouseButton(0))
        {
            ...
            //Camera.transform.SendMessage("setEnabled", false);
            _currIntersectPosition = _lastIntersectPosition;
            switch (_activeAxis)
            {
                case GIZMO_AXIS.X:
                    switch (_mode)
                    {
                        case GIZMO_MODE.TRANSLATE:
                            ...
                            {
                                _currIntersectPosition = ray.direction * hitDistance;
                        ...
                        case GIZMO_MODE.ROTATE:
                            ...
                            _XRotationDisplayValue = ClampRotationAngle(_XRotationDisplayValue);
                            break;
                ...
                case GIZMO_AXIS.Y:
                    switch (_mode)
                    {
                        ...
                        case GIZMO_MODE.ROTATE:
                            ...
                            _YRotationDisplayValue = ClampRotationAngle(_YRotationDisplayValue);
                            break;
                ...
                case GIZMO_AXIS.Z:
                    switch (_mode)
                    {
                        ...
                        case GIZMO_MODE.ROTATE:
                            ...
                            _ZRotationDisplayValue = ClampRotationAngle(_ZRotationDisplayValue);
                            break;
