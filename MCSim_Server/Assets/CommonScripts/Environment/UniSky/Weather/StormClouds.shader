Shader "Rain Clouds Layer" 
{
	Properties 
   {
		...
		_StormCenter("Storm Center", Vector) = (0,0,0)
   ...
   Subshader 
   {
	 Tags { "RenderType"="Geometry" "Queue" = "Geometry+3"}
		Fog { Mode Off }
	  Pass 
	  {
	...
	//	Offset 1,-10
			...
			v2f vert (appdata_v v)
			{
				...
				// distance to a storm cell
				o.dist = _StormCenter.xy - mul(v.vertex, _Object2World).xz;
			...
			{
				// Static settings for storm clouds
				_PrecipLevel = 0.25;
				...
				retColor.rgb = lerp(float3(1,1,1), retColor.rgb, 0.3);
				
				retColor.rgb *= _SunColor.rgb;
				
				// var (cloud color glow variance)
				retColor.a *= (5.3-retColor.r);
				...
				if(retColor.a > 0)
				{
				//	retColor.a *= 1 - ((length(i.dist)*length(i.dist))/7000000 * 2);
					retColor.a *= 1 - length(i.dist)/2000;
				}
