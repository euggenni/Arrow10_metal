...
public class ControlPanelToolkit : MonoBehaviour, Panel {
                ...
                switch ((int)enumvalue)
                {
                    case (int)ControlType.Tumbler:
                        for (int i = 0; i < panelgameobjects[enumvalue].Count; i++)
                        {
                            ...
                            if (panelgameobjects[enumvalue][i].GetComponent<SwitcherToolkit>() == null) continue; // Если на объекте висит SwitcherToolit, идем к следующему
                            else
                            {
                    ...
                    case (int)ControlType.Spinner:
                        for (int i = 0; i < panelgameobjects[enumvalue].Count; i++)
                        {
                            ...
                            if (panelgameobjects[enumvalue][i].GetComponent<SpinnerToolkit>() == null) continue; // Если на объекте висит SwitcherToolit, идем к следующему
                            else
                            {
                                Debug.Log("GameObject was succesfully delievered to HELL");
                            } // Иначе отправляем его в АД
                    ...
                    case (int)ControlType.Indicator:
                        for (int i = 0; i < panelgameobjects[enumvalue].Count; i++)
                        {
                            ...
                            if (panelgameobjects[enumvalue][i].GetComponent<IndicatorToolkit>() == null) continue; // Если на объекте висит IndicatorToolkit, идем к следующему
                            else
                            {
                                Debug.Log("GameObject was succesfully delievered to HELL");
                            } // Иначе отправляем его в АД
                    ...
                    case (int)ControlType.Joystick:
                        for (int i = 0; i < panelgameobjects[enumvalue].Count; i++)
                        {
                            ...
                            if (panelgameobjects[enumvalue][i].GetComponent<JoystickToolkit>() == null) continue; // Если на объекте висит JoystickToolkit, идем к следующему
                            else
                            {
                                Debug.Log("GameObject was succesfully delievered to HELL");
                            } // Иначе отправляем его в АД
    ...
    public void         SetGameObject(ControlType ObjectType, GameObject obj, int index)
    {
        ...
        if (panelgameobjects[ObjectType][index] != null)
            if (panelgameobjects[ObjectType][index].name == obj.name) return; // Если на этой позиции есть этот объект              
