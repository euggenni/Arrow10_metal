...
public class SpinnerToolkit : PanelControl
{
        ...
        set
        {
            if (value)
            {
                ...
                if (this.gameObject.GetComponent<SphereCollider>() != null)
                {
                    ...
                    catch {Debug.Log("Ошибка при удалении компонента");}
                }
            _isManual = value;
        }
    }   
    ...
    [SerializeField] private int minimalValue = 0;           // Минимальное значение
    ...
    [SerializeField] private int maximalValue = 100;         // Максимальное значение
    ...
    public void CalculateDimension()
    {
        ...
        if (this.startRotation.transform.localEulerAngles[(int)Axe] >= this.endRotation.transform.localEulerAngles[(int)Axe]) // Если начальный угол < конечного
