...
public class CoreToolkit : MonoBehaviour, CoreLibrary.Core {
    ...
    public void UnsubscribeFromOnControlChanged(CoreLibrary.OnControlChanged callback) {
        try {
            ...
            Debug.LogError("Ошибка при отписывании от события OnControlChanged");
        }
    }
    ...
    [SerializeField]
    private Object _libraryObject; // Объект с нужным скриптом-библиотекой

    [SerializeField]
    private GameObject _coreObject
        ; // Объект, на который во время виртуализации будут помещаться компоненты, связанные с Core 
        ...
        if (panelObject.GetComponent<ControlPanelToolkit>() != null) {
            ...
            if (script.Panel != null) {
                ...
                if (script.Panel.GetType().ToString() == Library.Panels[index].ToString()) {
                    ...
                    Debug.LogWarning("Type of this panel is [" + script.Panel.GetType() +
                                     "] doesnt equals required type [" +
    ...
    public void SetLibraryObject(Object obj) {
        if (obj == null) return;
        if (obj.Equals(_libraryObject)) {
            return;
        }
    ...
    public Object GetLibraryObject() {
    ...
    public ControlPanelToolkit GetPanel(string PanelName) {
        ...
        if (res == null) {
            Debug.Log("Panel with name [" + PanelName + "] was not found");
        }
    ...
    public void Virtualize() {
        ...
        _coreObject = (GameObject) Instantiate(Resources.Load("cubePrefab"), Vector3.zero, Quaternion.identity);
        _coreObject.name = "Core_" + Library.Name + "(Virtual)";
