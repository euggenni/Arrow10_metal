...
/// </summary>
[Serializable]
public class PanelObjectsList : Object {
    [SerializeField]
    ...
    public GameObject this[ControlType objectType, int index] {
        get {
            switch (objectType) {
                case ControlType.Tumbler:
                    Debug.Log("Tumblers count: index = " + index);
        ...
        set {
            switch (objectType) {
                case ControlType.Tumbler:
                    tumblers[index] = value;
                    break;

                case ControlType.Spinner:
                    spinners[index] = value;
                    break;
    ...
    public List<string> GetPanelDescriptions() {
        ...
        foreach (FieldInfo fieldInfo in GetType().GetFields()) {
            if (GetPanelNames().Contains(fieldInfo.FieldType.ToString())) {
                EnumDescription[] attrs =
                    (EnumDescription[]) fieldInfo.GetCustomAttributes(typeof(EnumDescription), false);
                if (attrs.Length > 0) {
...
public class EnumTumbler<EnumT> : Tumbler {
    ...
    public EnumTumbler(EnumT value) {
        ...
        var attributes =
            (DescriptionAttribute[]) typeof(EnumT).GetCustomAttributes(typeof(DescriptionAttribute), false);
...
public class StateEventArgs : EventArgs {
    ...
    public StateEventArgs(int port, bool state) {
        Port = port;
        State = state;
    }
}
