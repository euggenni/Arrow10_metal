...
public class ControlPanelToolkitEditor : Editor {    
    ...
    public void GetEditorSettings()
    {
        ...
        enableUndo = EditorPrefs.GetBool("enableUndo", true);
        useDarkSkin = EditorPrefs.GetBool("UseDarkSkin", true);
    }
        ...
        else
        {
            ...
            if (controlpanel.Panel != null)
            {
                ...
                foreach (ControlType value in Enum.GetValues(typeof(ControlType)))
                {
                    ...
                    //    Debug.Log("i = " + i + "; " + list_control_descriptions[i].ToString());
                    //}

                    //for (int i = 0; i < list_control_names.Count; i++) {
                    //    Debug.Log("j = " + i + "; " + list_control_names[i].ToString());
                    //}

                    var count_diff = list_control_names.Count - controlpanel.PanelGameObjects[value].Count;
                    for (var i = 0; i < count_diff; i++) {
                        controlpanel.PanelGameObjects[value].Add(null);
                    }

                    for (int i = 0; i < list_control_descriptions.Count; i++)
                    {
                        Debug.Log("[leonov]Iteration Number = "+ i);
                        ...
                        if (controlpanel.SwitcherScripts.Length < list_control_names.Count) {
                            var oldSwitcherScripts = controlpanel.SwitcherScripts;
                            controlpanel.SwitcherScripts = new SwitcherToolkit[list_control_names.Count];
                            for (var j = 0; j < list_control_names.Count; j++) {
                                if (j < list_control_names.Count) {
                                    controlpanel.SwitcherScripts[j] = oldSwitcherScripts[j];
                                } else {
                                    controlpanel.SwitcherScripts[j] = controlpanel.PanelGameObjects[value][j].GetComponent<SwitcherToolkit>();
                                }
                            }
                        }
