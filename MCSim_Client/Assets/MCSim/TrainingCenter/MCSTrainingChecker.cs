...
public class MCSTrainingChecker : MonoBehaviour {
    ...
    public void OnControlChanged(PanelControl control) {
        ...
        if (taskCommands.Count != 0) {
            if (CurrentCommand == null) {
                Debug.LogError("NULL CurrentCommand");
                return;
            }
            ...
            switch (control.ControlType) {
                case ControlType.Tumbler:
                    ...
                    if (CurrentCommand.PanelName == control.GetPanelName() &&
                        CurrentCommand.ControlName == control.GetName() && CurrentCommand.State.Equals(control.State)) {
                        if (taskCommands.Count > 1) {
                            ...
                            CallOnTaskAttempt(true, CurrentCommand,
                                taskCommands[1].Description); // Доложили об успешном выполнении
                            completedCommands.Add(CurrentCommand);
                            taskCommands.RemoveAt(0);
                            CallOnCurrentControlChanged(
                                Weaponry.Core[Order.PerformerName].GetPanel(Order.Commands[0].PanelName).GetControl(
                                    GetControlType(taskCommands[0]).ControlType, taskCommands[0].ControlName));

                            //BackPosition(trOrder, Weaponry);
                            ...
                            CallOnTaskAttempt(true, CurrentCommand,
                                CurrentCommand.Description); // Доложили об успешном выполнении
                            completedCommands.Add(CurrentCommand);
                            taskCommands.RemoveAt(0);
                            try {
                                CallOnCurrentControlChanged(Weaponry.Core[Order.PerformerName]
                                    .GetPanel(Order.Commands[0].PanelName)
                                    .GetControl(GetControlType(taskCommands[0]).ControlType,
                                        taskCommands[0].ControlName));
                            } catch (Exception r) {
                            }
                        }
                    } else {
                        CallOnTaskAttempt(false, CurrentCommand,
                            CurrentCommand.Description); // Доложили о неуспешном выполнении
                    }
                ...
                case ControlType.Spinner:
                    ...
                    try {
                        ...
                        CallOnTaskAttempt(false, CurrentCommand,
                            taskCommands[1].Description); // Доложили о неуспешном выполнении
                        return;
                    }

                    if (CurrentCommand.PanelName == control.GetPanelName() &&
                        CurrentCommand.ControlName == control.GetName() &&
                        (Convert.ToInt32(CurrentCommand.State) - a < Convert.ToInt32(control.State)) &&
                        (Convert.ToInt32(CurrentCommand.State) + a > Convert.ToInt32(control.State))) {
                        if (taskCommands.Count > 1) {
                            CallOnTaskAttempt(true, CurrentCommand,
                                taskCommands[1].Description); // Доложили о успешном выполнении
                            completedCommands.Add(CurrentCommand);
                            taskCommands.RemoveAt(0);
                            CallOnCurrentControlChanged(Weaponry.Core[Order.PerformerName]
                                .GetPanel(Order.Commands[0].PanelName)
                                .GetControl(GetControlType(taskCommands[0]).ControlType, taskCommands[0].ControlName));
                        } else {
                            CallOnTaskAttempt(true, CurrentCommand,
                                CurrentCommand.Description); // Доложили о успешном выполнении
                            completedCommands.Add(CurrentCommand);
                            taskCommands.RemoveAt(0);
                            CallOnCurrentControlChanged(Weaponry.Core[Order.PerformerName]
                                .GetPanel(Order.Commands[0].PanelName)
    ...
    public Vector3 GetCurrentControlPosition(OrderSubtask order) {
        return _core.GetPanel(order.PanelName).GetControl(GetControlType(order).ControlType, order.ControlName)
            .transform.position;
    }
