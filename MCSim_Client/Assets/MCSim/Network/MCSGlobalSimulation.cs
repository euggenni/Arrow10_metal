...
public class MCSGlobalSimulation : MonoBehaviour {
    public static List<GameObject> CloudPointsWhite = new List<GameObject>();
    public static List<GameObject> CloudPointsGrey = new List<GameObject>();
    public static List<Collider> Cloud = new List<Collider>();
    ...
    /// </summary>
    public static float
        SimulationStartedTime // Впринципе, всегда равен нулю, если мы начинаем симуляцию с момента загрузки уровня
    {
    ...
    public static Weaponry Instantiate(GameObject go) {
        ...
        if (go != null) {
            if (weaponry = go.GetComponent<Weaponry>()) {
                ...
                if (weaponry)
                    CommandCenter.Execute(new MCSCommand(MCSCommandType.Simulation, "Instantiate", true,
                        weaponry.GetType().FullName, weaponry.ID));
            } else {
    ...
    public static Weaponry Instantiate(string weaponryType, NetworkPlayer player) {
        ...
        try {
            ...
            // Если экземпляр создан успешно
            if (weaponry)
                CommandCenter.Execute(player,
                    new MCSCommand(MCSCommandType.Simulation, "Instantiate", true, weaponry.GetType().FullName,
                        weaponry.ID));
        } catch {
    ...
    public static Weaponry ClientInstantiate(int weaponryID, string weaponryType) {
        ...
        if (weaponry.gameObject.GetComponent<WeaponryPlane_Cloud>() != null ||
            weaponry.gameObject.GetComponent<WeaponryPlane_Cloud_2>() != null) {
            Cloud.Add(weaponry.collider);
        } else {
            if (weaponry.gameObject.GetComponent<WeaponryPlane_TargetCloud>() != null) {
                CloudPointsWhite.Add(weaponry.gameObject);
            } else {
                if (weaponry.gameObject.GetComponent<WeaponryPlane_TargetCloud_2>() != null) {
                    CloudPointsGrey.Add(weaponry.gameObject);
                }
            }
        }
        Weapons.List.Add(weaponry.ID, weaponry); // Добавляем в список
        ...
        CommandCenter.Execute(RPCMode.Server, newCmd);
        //        }
        //        catch
        //        {
        //            Debug.LogWarning("Ошибка при создании экземпляра Weaponry.");
    ...
    public static Weaponry InstantiateAI(string weaponryType) {
        ...
        try {
            ...
            Debug.LogError("Ошибка при создании AI для Weaponry [" + weaponryType +
                           "]. Для данного Weaponry не определен интерфейс AIControllable.");
            return null;
        }
    ...
    public static void InitializeServer() {
        GameObject go = (GameObject) Network.Instantiate(Resources.Load("MCSim/Network/MCSCommandCenter"), Vector3.zero,
