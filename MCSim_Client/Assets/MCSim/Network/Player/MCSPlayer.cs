...
public class MCSPlayer {
    ...
    /// <summary>
    /// Возвращает аккаунт текущего игрока
    /// </summary>
    public static MCSPlayer Me {
        get {
            foreach (var value in MCSGlobalSimulation.Players.List.Values) {
                if (value.NetworkPlayer.Equals(Network.player)) return value;
            }

            Debug.LogError("Не удалось найти MCSPlayer текущего игрока");
            return null;
        }
    }
