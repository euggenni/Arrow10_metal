...
public class TankTrackController : MonoBehaviour {
    ...
    public void UpdateWheels(float accel, float steer) {
        ...
        leftTrackTextureOffset =
            Mathf.Repeat(leftTrackTextureOffset + delta * trackRpm * trackTextureSpeed / 60.0f, 1.0f);
        ...
        rightTrackTextureOffset =
            Mathf.Repeat(rightTrackTextureOffset + delta * trackRpm * trackTextureSpeed / 60.0f, 1.0f);
    ...
    public void CalculateMotorForce(WheelCollider col, float accel, float steer) {
        ...
        if (accel == 0 && steer == 0) {
            ...
            if (steer < 0) {
                ...
                col.motorTorque = steer * forwardTorque * rotateOnMoveMultiply * 1.33f; //13 
                fc.stiffness = 1.0f + minOnMoveStiffness - Mathf.Abs(steer); //14 
            }

            if (steer > 0) {
                //15                 
                col.motorTorque = steer * forwardTorque * rotateOnMoveMultiply * 1.33f; //16 
